...
function NewReview() {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 pb-4">
      <h1 className="text-xl font-bold">סיכום יומי</h1>
      ...
      <Card className="p-4 gradient-card space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Field label="P&L יומי"><Input type="number" step="0.01" value={f.total_pnl} onChange={(e) => set("total_pnl", e.target.value)} /></Field>
          <Field label="מס' עסקאות"><Input type="number" value={f.trades_count} onChange={(e) => set("trades_count", e.target.value)} /></Field>
        </div>
      ...
      <Card className="p-4 gradient-card space-y-3">
        ...
        <Field label="קטליסט עיקרי היום"><Input value={f.main_catalyst} onChange={(e) => set("main_catalyst", e.target.value)} /></Field>
        <Field label="הקשר השוק"><Textarea rows={2} value={f.market_context} onChange={(e) => set("market_context", e.target.value)} /></Field>
      </Card>
      ...
      <Card className="p-4 gradient-card space-y-3">
        ...
        <Field label="חוק שאני חייב לשמור עליו מחר"><Textarea rows={2} value={f.rule_for_tomorrow} onChange={(e) => set("rule_for_tomorrow", e.target.value)} /></Field>
        <div className="flex items-center justify-between rounded-lg bg-input/40 px-3 py-2">
          <Label className="text-sm">להקטין סייז מחר?</Label>
      ...
      <Button type="submit" disabled={saving} className="w-full h-12 text-base font-bold">
        <Save className="h-4 w-4 ml-2" />{saving ? "שומר..." : "שמור סיכום"}
      </Button>
    </form>
  );
}
